import type { Metadata } from 'next';
import Link from 'next/link';
import { FadeIn } from '@/components/ui/FadeIn';
import { SectionLabel } from '@/components/ui/SectionLabel';

export const metadata: Metadata = {
  title: 'Web Applications',
  description:
    'Custom web applications, client portals, booking systems and dashboards built around real business workflows. Developed by TheoMedia for UK, Ireland and Europe.',
};

const features = [
  { title: 'Client Portals', description: 'Secure login areas where customers manage accounts, orders and communications.' },
  { title: 'Booking Systems', description: 'Table reservations, room booking, consultation scheduling and appointment flows.' },
  { title: 'Staff Dashboards', description: 'Real-time operational dashboards that replace spreadsheets and manual tracking.' },
  { title: 'Secure Authentication', description: 'Role-based access, multi-factor authentication and session management.' },
  { title: 'Real-Time Data', description: 'Live updates, notifications and data synchronisation across devices.' },
  { title: 'API Integration', description: 'Connection to payment providers, CRMs, booking platforms and third-party services.' },
];

export default function WebApplicationsPage() {
  return (
    <>
      <section className="bg-near-black text-bone pt-32 pb-20 md:pt-40 md:pb-28 px-5 md:px-8 lg:px-12">
        <div className="max-w-[1440px] mx-auto">
          <SectionLabel dark>SERVICES</SectionLabel>
          <h1 className="text-editorial-xl text-bone mt-4 mb-6 max-w-[800px]">
            Web Applications
          </h1>
          <p className="text-[16px] md:text-[18px] text-bone/60 leading-relaxed max-w-[560px]">
            Portals, dashboards and business tools designed around real workflows.
          </p>
        </div>
      </section>

      <section className="bg-bone py-20 md:py-28 px-5 md:px-8 lg:px-12">
        <div className="max-w-[1440px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {features.map((feature, i) => (
              <FadeIn key={feature.title} delay={i * 0.05}>
                <div className="py-6 border-b border-near-black/10">
                  <h3 className="text-[18px] md:text-[20px] font-sans font-semibold text-near-black mb-2">
                    {feature.title}
                  </h3>
                  <p className="text-[15px] text-stone leading-relaxed">
                    {feature.description}
                  </p>
                </div>
              </FadeIn>
            ))}
          </div>
        </div>
      </section>

      <section className="bg-ivory py-20 md:py-28 px-5 md:px-8 lg:px-12">
        <div className="max-w-[1440px] mx-auto text-center">
          <h2 className="text-editorial-md text-near-black mb-6">
            Need something custom?
          </h2>
          <p className="text-[16px] text-stone mb-8 max-w-[480px] mx-auto">
            From client portals to operational dashboards, we build tools people actually use.
          </p>
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 px-8 py-4 bg-near-black text-bone text-[13px] font-sans font-medium tracking-[0.1em] uppercase hover:bg-charcoal transition-colors"
          >
            Start a Project →
          </Link>
        </div>
      </section>
    </>
  );
}
